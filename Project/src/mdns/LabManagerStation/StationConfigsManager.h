...
#define HOSTNAME_MAX_LENGTH 128                   // Max Length Of A HostName
...
String sonoff_hostname = "";                    //Static IP of Sonoff Plug
// 
...
bool saveConfigFile()
{
  ...
  json["sonoff_hostname"] = sonoff_hostname;
...
void saveCustomParameters(
  ...
  WiFiManagerParameter* custom_sonoff_hostname
) {
  ...
  //SONOFF IP Address
  sonoff_hostname = String(custom_sonoff_hostname->getValue());
  Serial.print("SONOFF IP ADDRESS: ");
  Serial.println(sonoff_hostname);
    ...
    if (SPIFFS.exists(JSON_CONFIG_FILE)) { // The file exists, reading and loading
      ...
      if (configFile) {
        ...
        if (!error) {
          ...
          sonoff_hostname = json["sonoff_hostname"].as<String>();
 ...
 */
bool updateConfigFile(String newOwnerID, String newStationName, int newRunTime, String newAccessibility, String newStationStatus, String newSonoffHostName) {
  ...
  String oldSonoffHostName = sonoff_hostname;
  ...
  sonoff_hostname = newSonoffHostName;
  // update the configs
  ...
  if(!result) {
    ...
    sonoff_hostname = oldSonoffHostName;
...
//  wm.resetSettings();                             // Reset settings (only for development) <---- Comment it out when finishing development!
  ...
  // SONOFF IP ADDRESS
    char convertedSonoffHostNameValue[HOSTNAME_MAX_LENGTH];
    sprintf(convertedSonoffHostNameValue, "%s", sonoff_hostname.c_str());
    WiFiManagerParameter custom_sonoff_hostname("sonoff_hostname", "Sonoff Host Name: ", convertedSonoffHostNameValue, HOSTNAME_MAX_LENGTH);
         
  // Add all defined parameters
  wm.addParameter(&custom_station_id);
  wm.addParameter(&custom_sonoff_hostname);
  ...
  //Save custom parmeters
  // If we get here, we are connected to the WiFi
  saveCustomParameters(&custom_station_id, &custom_sonoff_hostname); 
}
